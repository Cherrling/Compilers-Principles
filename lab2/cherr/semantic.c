...
void FunDec(ast_node *node,Type spec,int state)
{
    ...
    //    VarList     : ParamDec COMMA VarList 
                //    | ParamDec                    
                //    ;
        ast_node * varList = node->child[2];
        while(1)
        {
            // ParamDec    : Specifier VarDec
            ast_node * paramDec =varList->child[0];
...
void CompSt(ast_node *node,Type ftype)
{
    ...
    ////
    ast_node * stmtlist =node->child[2];
...
//         | FLOAT             1.0
    ...
    {//ID
    //找变量
        FieldList field = search(node->child[0]->val.str_value,0);
        if(field == NULL)
        {
            printf("Error type 1 at Line %d: Undefined variable %s.\n",node->lineno, node->child[0]->val.str_value);
            return NULL;
        }
        return field->type;
    }
...
//         | Exp LB Exp RB     a[b]  左值
        ...
        ( strcmp(node->child[1]->name, "LB") == 0))
    {
        ...
        {
            //todo : 看这个变量是否被定义
            printf("Error type 10 at Line %d: %s is not an array.\n", node->lineno, exp1->child[0]->val.str_value);
            return NULL;
        }
        ...
        strcmp(node->child[1]->name, "DOT") == 0))
    {
        ...
        char * str = node->child[2]->val.str_value; 
        while(p!=NULL)
        {
        ...
        printf("Error type 14 at Line %d: Non-existent field %s.\n", node->lineno, node->child[2]->val.str_value);
        return NULL;
    }
...
Type Specifier(ast_node *node)
{
    ...
    if(strcmp(node->child[0]->name,"TYPE")==0)
    {
        ...
        spec->kind = BASIC;
        if(strcmp(node->child[0]->val.str_value,"float")==0)
...
Type StructSpecifier(ast_node *node)
{
    ...
    if(node->child[1] !=NULL && strcmp(node -> child[1]->name ,"Tag") == 0 )
    {
        ...
        // ->val.str_value;
        //printf("gotoTag\n");
        ast_node * Tag = node->child[1];
        ast_node * ID = Tag->child[0];
        char *str = ID->val.str_value;
        //找结构体类型
    ...
    //这里作用域在struct内
    enter_scope();
    ast_node * defList = node->child[3];
    spec->u.structure = NULL;
    while(defList != NULL)
    {//Def DefList
        ast_node *def = defList->child[0];
        // Def     : Specifier DecList SEMI
        Type t = Specifier(def->child[0]);
        ast_node * decList = def->child[1];
        // DecList : Dec                 
        //         | Dec COMMA DecList        
        //         ;
        ...
        while(1)
        {
            ...
            //         | VarDec ASSIGNOP Exp  a = 5
            //         ;
            ast_node * dec = decList->child[0];
            ast_node * varDec = dec->child[0];
...
//结构体类型名 必须唯一
{
    ...
    char *s =node->child[0]->val.str_value;
    field->name = s;
